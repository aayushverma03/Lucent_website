...
| Category | What we use them for | Do you need to consent? |
|---|---|---|
| **Strictly necessary** | Keep you signed in, prevent fraud, remember your privacy choices, keep the site secure. | No — these are essential. We disclose them but don't ask consent. |
| **Functional / preference** | Remember your language, region, theme, and similar preferences. | **Yes** — opt-in. |
| **Analytics / performance** | Measure how the site is used so we can fix bugs and improve features. | **Yes** — opt-in. |
| **Marketing / advertising** | Show relevant content on other sites, measure campaign performance. | **Yes** — opt-in, separately from analytics. |
...
- **Strictly necessary SDK identifiers** to keep you logged in and protect the app.
- **Analytics SDKs** to understand how features are used. These run only if you opt in.
- **Crash and diagnostic SDKs** to fix bugs. These collect only device and crash data, not personal content.
- **Advertising identifiers** (Apple's IDFA, Android's Advertising ID) — these are **off by default** and only used if you opt in.
